...
if (isset($_GET['ids']) && !empty($_GET['ids'])) {
    ...
    foreach ($selectedIds as $id) {
        $categoryQuery = mysqli_query($conn, "SELECT category_id, image FROM addons WHERE id = '$id'");
    ...
    foreach ($selectedIds as $id) {
        ...
        $deleteProductQuery = "DELETE FROM addons WHERE id = ?";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            echo "Error deleting add-ons with ID $id: " . mysqli_error($conn);
        }
    }
    ...
    session_start();
    $_SESSION['success_message'] = "Selected add-ons were successfully deleted.";

    // Redirect back to the product list page
    header('Location: add-addons.php');
    exit();
} else {
