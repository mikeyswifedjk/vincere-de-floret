...
if (isset($_GET['id'])) {
    ...
    $selectProductQuery = "SELECT * FROM addons WHERE id = ?";
    ...
    if (mysqli_stmt_execute($stmtProduct)) {
        ...
        echo "Error fetching addons details: " . mysqli_stmt_error($stmtProduct);
    }
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                if (move_uploaded_file($tempName, $uploadDir . $newImageName)) {
                    ...
                    $updateProductQuery = "UPDATE addons SET name = ?, category = ?, image = ?, price = ?, qty = ? WHERE id = ?";
                ...
                $updateProductQuery = "UPDATE addons SET name = ?, category = ?, price = ?, qty = ? WHERE id = ?";
                $stmt = mysqli_prepare($conn, $updateProductQuery);
                mysqli_stmt_bind_param($stmt, 'ssssi', $newName, $newCategory, $newPrice, $newQty, $id);
                mysqli_stmt_execute($stmt);
            }

            echo "<script>alert('Add-Ons Updated Successfully'); document.location.href = 'add-addons.php';</script>";
        }
    } else {
        echo '<script>alert("Add-Ons not found with ID: ' . $id . '");</script>';
    }
} else {
    echo '<script>alert("Add-Ons ID not provided");</script>';
}
?>
...
<body>
    ...
    <div class="all">
        <h1 class="title">Update Add-Ons</h1>
